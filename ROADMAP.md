...
## Phase 4 — Broader framework integrations ✅ shipped in v0.8.0

> **Goal:** Let the library work without modification in the most common Python server and worker runtimes.

### 4.1 Django middleware ✅ shipped in v0.8.0

`RuntimeNarrativeDjangoMiddleware` (async ASGI) and `RuntimeNarrativeDjangoSyncMiddleware` (sync WSGI). Each request becomes a story named `"METHOD /path"`. Requires `[django]` extra.

### 4.2 Celery integration ✅ shipped in v0.8.0

`NarrativeTask` base class wraps each task execution in a story. Story name includes the task name and Celery task ID. `connect_narrative(app, ...)` sets defaults globally. Requires `[celery]` extra.

### 4.3 `asyncio.TaskGroup` support ✅ shipped in v0.8.0

`NarrativeTaskGroup` runs concurrent asyncio tasks under a shared story. Tasks inherit the story context automatically. Failed tasks are reported via `NarrativeTaskGroupError`. No extra dependencies; Python 3.9+.

### 4.4 gRPC interceptor ✅ shipped in v0.8.0

`RuntimeNarrativeInterceptor` (sync) and `RuntimeNarrativeAsyncInterceptor` (async) wrap each RPC in a story. Requires `[grpc]` extra.
...
**New features — Phase 4 broader framework integrations**

- `RuntimeNarrativeDjangoMiddleware` / `RuntimeNarrativeDjangoSyncMiddleware` — Django ASGI/WSGI middleware (`[django]` extra).
- `NarrativeTask` + `connect_narrative` — Celery task base class integration (`[celery]` extra).
- `NarrativeTaskGroup` + `NarrativeTaskGroupError` — concurrent asyncio task tracking under a shared story. No extra dependencies.
- `RuntimeNarrativeInterceptor` + `RuntimeNarrativeAsyncInterceptor` — sync and async gRPC server interceptors (`[grpc]` extra).
