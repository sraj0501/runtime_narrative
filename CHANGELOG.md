...
### Fixed
- **`ConsoleRenderer` no longer prints two timestamps on `LogRecorded` lines when the `structlog` extra is installed.** `handle()` was passing a `"timestamp"` key into the `event_dict` handed to `structlog.dev.ConsoleRenderer`, which rendered it in addition to the timestamp already printed by the story tag (`_story_tag()`) at the start of the line. The `event_dict` no longer carries `timestamp` — the story tag's timestamp is the only one shown. (#41)
